...
const figmaLinks: Record<string, string> = {
  'Foodie': 'https://www.figma.com/design/EPgQRHbPGnggCNvfOZ8S4z/foodie?node-id=0-1&t=UgBlteZLDzXobERk-1',
  'Fashion': 'https://www.figma.com/design/wYVJa7ec0lbZLE4O1hmDZ9/fashion?node-id=1-1425&t=R1Fg0vUhYFQcWYgf-1',
  'CleanUp': 'https://www.figma.com/design/qKCuje5QeOsRoGmLwsWdM1/CleanUP?node-id=1-16355&t=FCwPc3RvNyI7Gu89-1',
};

const ProjectModal = ({ project, onClose }: ProjectModalProps) => {
  const accentColor = project.projectType === 'ui/ux' ? 'var(--accent-teal)' : 'var(--accent-yellow)';
  const figmaLink = figmaLinks[project.title];
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="relative w-full max-w-4xl max-h-[90vh] bg-[var(--bg-mid)] rounded-lg shadow-2xl overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-800"
      ...
      >
        ...
        <div className="p-8 md:p-12">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[var(--text-muted)] mb-6">
            ...
            {figmaLink && (
              <a
                ...
                className="inline-flex items-center px-4 py-1.5 text-sm font-semibold text-black rounded-full transition-all duration-300 hover:scale-105"
            ...
            {project.caseStudy.type === 'ui/ux' && (
              <>
                {project.caseStudy.appPreviewVideo ? (
                  <div className="mb-6">
                    <h3 className="text-xl font-bold text-[var(--text-light)] mb-4">
                      App Preview
                    </h3>
          ...
          {(() => {
            const items = project.caseStudy.type === 'ui/ux' 
              ? project.caseStudy.screens
              : project.caseStudy.renders;
            
            if (!items || items.length === 0) return null;

            const isSingleVideo = items.length === 1 && items[0].includes('.mp4');
                ...
                {isSingleVideo ? (
                  <div className="not-prose flex justify-center">
                    <video
                      ...
                      muted
                      playsInline
                      aria-label={`${project.title} final render`}
                    />
                  </div>
                ) : (
                  <div className="flex justify-center overflow-x-auto space-x-4 p-4 -mx-4 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-800">
                    {items.map((src, index) => (
                      <div key={index} className="flex-shrink-0 w-4/5 md:w-3/5 rounded-lg overflow-hidden shadow-lg bg-black">
                        {src.includes('.mp4') ? (
                          <video
                            src={src}
                          ...
                          <img 
                            src={src} 
                            alt={`${project.caseStudy.type === 'ui/ux' ? 'Screen' : 'Render'} ${index + 1}`} 
                            className="w-full h-auto object-cover"
                          />
                        )}
                      </div>
